...
'''   train AFM   '''
def train_AFM(parameter, criteo_sampled_data_path, criteo_name, wdl_visual):
    print('AFM 模型训练开始 ', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())))
    ...
    model = AFM(linear_feature_columns, dnn_feature_columns,
                ...
                use_attention=True, attention_factor=parameter['attention_factor'], l2_reg_att=parameter['l2_reg_att'], afm_dropout=parameter['afm_dropout'],
                task='binary')
    ...
    # # 模型保存成yaml文件
    # save_model(model, save_path_AFM)
    ...
    print(('AFM 模型训练运行时间： {:.0f}分 {:.0f}秒'.format((end_time_end - start_time_start) // 60,
...
if __name__ == '__main__':
    ...
    train_AFM(parameter, criteo_sampled_data_path, criteo_name, visual.format('AFM'))
