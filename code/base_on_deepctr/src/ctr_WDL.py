...
'''   train WDL   '''
def train_WDL(parameter, criteo_sampled_data_path, criteo_name, wdl_visual):
    print('WDL 模型训练开始 ', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())))
    ...
    # train
    model = WDL(linear_feature_columns=sparse_feature_columns, dnn_feature_columns=dense_feature_columns,
    ...
    # # 模型保存成yaml文件
    # save_model(model, save_path_WDL)
    ...
    print(('WDL 模型训练运行时间： {:.0f}分 {:.0f}秒'.format((end_time_end - start_time_start) // 60,
...
if __name__ == '__main__':
    ...
    train_WDL(parameter, criteo_sampled_data_path, criteo_name, visual.format('WDL'))
