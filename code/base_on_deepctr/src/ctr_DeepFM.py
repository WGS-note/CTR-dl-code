...
'''   train DeepFM   '''
def train_DeepFM(parameter, criteo_sampled_data_path, criteo_name, wdl_visual):
    ...
    # mNadam = SGD(learning_rate=parameter['lr'])
    # mNadam = Ftrl(learning_rate=parameter['lr'])
    ...
    # # 模型保存成yaml文件
    # save_model(model, save_path_DeepFM)
    ...
    print(('DeepFM 模型训练运行时间： {:.0f}分 {:.0f}秒'.format((end_time_end - start_time_start) // 60,
...
if __name__ == '__main__':
    ...
    train_DeepFM(parameter, criteo_sampled_data_path, criteo_name, visual.format('DeepFM'))
