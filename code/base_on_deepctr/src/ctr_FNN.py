...
'''   train FNN   '''
def train_FNN(parameter, criteo_sampled_data_path, criteo_name, fnn_visual):
    print('FNN 模型训练开始 ', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())))
    ...
    # train
    model = FNN(linear_feature_columns=sparse_feature_columns, dnn_feature_columns=dense_feature_columns,
    ...
    # # 模型保存成yaml文件
    # save_model(model, save_path_FNN)
    # print(' 模型保存完成', time.strftime("%H:%M:%S", time.localtime(time.time())))

    # 训练可视化
    # visualization(history, parameter['epochs'], saveflag=True, showflag=False, path=fnn_visual)
    ...
    print(('FNN 模型训练运行时间： {:.0f}分 {:.0f}秒'.format((end_time_end - start_time_start) // 60, (end_time_end - start_time_start) % 60)))
...
if __name__ == '__main__':
    ...
    train_FNN(parameter, criteo_sampled_data_path, criteo_name, visual.format('FNN'))
